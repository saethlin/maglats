...
if boundary == 0:
    ...
    if ndim == 2:
        spin[...,0] = f * np.prod(np.sin(np.pi*(np.mgrid[:Nz,:Nz])/(Nz-1)),0)
if afm:
...
np.save(history, np.array([name,afm,beta,GSE,en_max,1+periods*(rkstep//skip+1)]))
...
for period in range(periods):
    ...
    stdout.write(' '*78+'\r')
    rem = (now()-start_time)*(periods-period-1)
    stdout.write('Remaining: '+str(rem)+'\r')
    stdout.flush()
